...
/** Generous: score is bugs killed; allow bursty splat farming. */
...
/** Per-run HMAC proof: HMAC(salt, "submit|score|name|goldenStool"). */
function clientProof(salt, score, name, goldenStool) {
  return crypto
    .createHmac('sha256', salt)
    .update(`submit|${score}|${name}|${goldenStool}`)
    .digest('hex');
}

function maxPlausibleScore(elapsedSec) {
  const t = Math.max(1, elapsedSec);
  return Math.floor(MAX_POINTS_PER_SEC * t + 20);
}
...
/**
 * @param {{ name: unknown, score: unknown, goldenStool: unknown, token: unknown, proof: unknown }} body
 */
export function addScoreSecure(body) {
  const name = sanitizeName(body.name);
  const score = Math.floor(Number(body.score));
  const goldenStool = Math.floor(Number(body.goldenStool ?? 0));
  ...
  if (!Number.isFinite(goldenStool) || goldenStool < 0 || goldenStool > 1_000_000) {
    return { scores: listScores(), added: false, error: 'invalid_golden' };
  }
  ...
  const expectProof = clientProof(meta.salt, score, name, goldenStool);
  ...
  if (score > maxPlausibleScore(elapsedSec)) {
